...
### Michael's solo Zoom smoke test

The agent started a fresh instant Zoom meeting with personal meeting ID disabled and no invitations. Michael completed the short test and kept it as “Testing out the new meeting recording system”. Meeting `27239526-e960-422a-8dec-659eb298d5bf` ran from `2026-09-17T05:33:07.438Z` to `05:33:37.132Z`, lasting 29.694 seconds. Local video, microphone, and incoming-audio files were written, then the accepted revision 2 reached Bruce as production job 2. The existing managed child claimed it automatically. The Keep button was used; this run does not re-prove the ignored-prompt path. It also does not establish remote-speaker audio or shared-screen readability.

The first processing attempt exposed very slow library loading under launchd's `ProcessType=Background` and `LowPriorityIO=true`. After several minutes, Python had used only a few seconds of CPU, was mostly in uninterruptible wait, and remained around 48–70 MB RSS. A native sample showed Rust tokenizer initialization rather than a permission wait; TCC logs showed no new denial. A bounded `taskpolicy -B` diagnostic alone did not resolve the delay. Screen Sharing was locked; no remote UI or privacy change was made.

The installer now uses `ProcessType=Standard` and omits `LowPriorityIO`. It retains `nice 10`, one heavy job, the service lock, and all two-thread limits. The parsed-plist regression failed before the change and all five service-script tests passed afterward. Apple documents Background resource limits and a separate low-priority filesystem policy in [launchd.plist](https://github.com/apple-oss-distributions/launchd/blob/main/man/launchd.plist.5); [getiopolicy_np](https://developer.apple.com/library/archive/documentation/System/Conceptual/ManPages_iPhoneOS/man3/getiopolicy_np.3.html) describes delayed throttled reads, including on external disks.

Before retrying, both old processes were confirmed stopped, the lock was released, and an SQLite backup was saved under `validation/user-smoke-20260917/`. The exact job's existing lease was failed through the fenced queue API, then released through its normal retry API. No archive was deleted, new meeting fabricated, or successful job requeued. The new managed child claimed job 2 on attempt 2 at normal process priority while retaining nice 10. Library loading progressed substantially faster. Native samples are saved alongside the database backup; this was a recovered run after an agent-applied configuration fix, not a clean first-attempt unattended test.

Processing then succeeded on attempt 2 and Notion publication succeeded on its first attempt. The transcript contains eight microphone turns from one unknown speaker, including Michael's spoken introduction and statement that he was ending the meeting. The name was transcribed as “Mike Kan”; speaker identity was not automatically enrolled. All four original manifest files still verify. Playback probes as 1920 × 1080 H.264 with stereo 48 kHz AAC and 29.310 seconds of media. The accepted meeting's wall-clock duration is 29.694 seconds.

Read-only Notion API verification confirmed the active [Testing out the new meeting recording system](https://app.notion.com/p/Testing-out-the-new-meeting-recording-system-3defd70ecfa081dd9e57d0b0ea8969c1) page, 11 blocks, and the spoken test text. Final processing/publication state and read-back evidence are in `validation/user-smoke-20260917/result.json`. The private HTTPS viewer remains inactive pending approval; the transcript publication is verified, but its video-link route is not. A fresh meeting with no corrective intervention, remote audio, shared-screen readability, human speaker enrollment/reuse, and an actual reboot remain outstanding.
